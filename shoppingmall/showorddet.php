        ...
        <div class="mypage-information">
            ...
            <div class="bodys">
                <table id="items">
                    ...
                    $ordno = $_GET['ordno'];
                    include_once('dbconn.php');
                    $sql = "select orditem.*, size from orditem, shop_data 
            where ordno = '$ordno' and itemname = item order by seq";
                    $result = $conn->query($sql);
                    if (!$result)
                        die('주문상세내역 검색 오류 : ' . $conn->error);
                    ...
                    $ordno = $_GET['ordno'];
                    include_once('dbconn.php');
                    $sql = "select orditem.*, size from orditem, recommended 
            where ordno = '$ordno' and itemname = item order by seq";
                    $result = $conn->query($sql);
                    if (!$result)
                        die('주문상세내역 검색 오류 : ' . $conn->error);
                    ...
                    $ordno = $_GET['ordno'];
                    include_once('dbconn.php');
                    $sql = "select orditem.*, size from orditem, special 
            where ordno = '$ordno' and itemname = item order by seq";
                    $result = $conn->query($sql);
                    if (!$result)
                        die('주문상세내역 검색 오류 : ' . $conn->error);
                ...
                <a href="mypage.php" class="back">뒤로가기</a>
            </div>
        </div>
    </article>
